...
export const ItineraryTimeline = () => {
    ...
    return (
        <div className="w-full max-w-4xl mx-auto px-4 py-12">
            <div className="relative border-l-2 border-white/20 ml-4 md:ml-8 space-y-12">
                {state.itinerary.map((item, index) => (
                    <div key={index} className="relative pl-8 md:pl-12">
                        {/* Day Marker */}
                        <div className="absolute -left-3 md:-left-4 top-0 w-6 h-6 md:w-8 md:h-8 bg-amber-500 rounded-full flex items-center justify-center text-black font-bold text-xs md:text-sm shadow-[0_0_15px_rgba(245,158,11,0.5)]">
                            {item.day}
                        </div>
