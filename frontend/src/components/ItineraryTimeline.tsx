...
const DayCard = memo(({ item, destination, vibe }: DayCardProps) => {
    const mapsUrl = `https://maps.google.com/?q=${encodeURIComponent(`${destination} ${item.title}`)}`;

    return (
        <div className="relative pl-8 md:pl-12" role="listitem">
            <div
            ...
            <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-3xl p-6 md:p-8 hover:bg-white/10 transition-all duration-300 group">
                ...
                <div className="mt-6 flex items-center justify-between">
                    ...
                    <a
                        ...
                        className="flex items-center gap-1.5 text-xs font-semibold text-white/40 hover:text-amber-400 transition-colors"
                    >
        ...
        <section className="w-full max-w-4xl mx-auto px-4 py-12" aria-label="Travel itinerary">
            ...
            <div className="w-full h-48 md:h-64 rounded-3xl overflow-hidden mb-12 relative group shadow-2xl">
                <img
                    ...
                    alt={state.destination ? `Travel destination photo for ${state.destination}` : 'Travel destination photo'}
                    loading="lazy"
                    className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-theme-bg via-transparent to-transparent" aria-hidden="true"></div>
